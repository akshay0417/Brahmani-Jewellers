...
import { LogOut, User as UserIcon, Shield, Clock, ExternalLink, ShoppingBag, MessageSquare } from 'lucide-react';
  ...
  const handleSupportSubmit = async (e) => {
    ...
    try {
      await api.post('/messages', {
        name: user?.name || 'Logged-in User',
        email: user?.email || 'N/A',
        message: supportMessage
      });
      setSupportMessage('');
      setSupportStatus({ type: 'success', message: 'Your message has been sent to the admin successfully!' });
    } catch (err) {
      setSupportStatus({ type: 'error', message: err.response?.data?.message || 'Error sending message' });
    } finally {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            ...
            <motion.div
              ...
              className="bg-cream-alt border border-ochre/10 p-8 rounded-lg shadow-sm transition-colors duration-300"
            >
            ...
            >
              <div className="flex items-center gap-3 mb-6 border-b border-ochre/10 pb-4">
                <MessageSquare className="text-ochre" size={24} />
                <h2 className="text-xl font-serif font-bold text-coffee uppercase tracking-widest">Contact Admin</h2>
              </div>

              {supportStatus.message && (
                <div className={`mb-4 p-3 rounded text-sm ${supportStatus.type === 'error' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}>
                  {supportStatus.message}
                </div>
              )}
              ...
              <form onSubmit={handleSupportSubmit} className="space-y-4">
                <div>
                  <label className="text-xs text-coffee/60 uppercase tracking-widest mb-1 block">Your Message</label>
                  <textarea
                    rows={4}
                    value={supportMessage}
                    onChange={(e) => setSupportMessage(e.target.value)}
                    placeholder="Ask a question or send feedback to the admin..."
                    className="w-full bg-cream border border-ochre/30 rounded px-3 py-2 text-coffee focus:outline-none focus:border-ochre text-sm"
                    required
                  />
                </div>
                <button
                  ...
                  className="w-full py-2 bg-ochre hover:bg-ochre/90 text-coffee font-bold uppercase text-xs tracking-wider rounded shadow transition-all disabled:opacity-50"
                >
